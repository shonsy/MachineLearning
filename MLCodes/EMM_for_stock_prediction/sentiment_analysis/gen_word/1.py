...
class LR_Uni_Bi():
    ...
    def buildUnigram(self, min1=0, sw1=True):
        ...
        count = 0                                 ### ����Ϊ�٣�if�µ���䲻��ִ�аɣ�
        
        for word in temp_set:
            self.dic[word] = count
            count += 1                           ### ����set()ֻ�洢���ظ���Ԫ�أ���word��ӵ�temp_set�У���ÿ��word�Ĵ������Ƕ�Ϊ1����
    ...
    def buildBigram(self, min2=5, sw2=True):
        ...
        '''
        self.gram2 = {}
            # ��ʱ�������洢bigram�Ĵ���������min-count����
        for fname in os.listdir(self.train_dir):
            for line in open(os.path.join(self.train_dir, fname)):
                doc = line.split()
                for i in range(len(doc)-(2-1)):             ### range()������ʵ��len(doc)-1,������1��len(doc)-2����˼�𣿲�������ʲô���ã�
                    t = tuple(doc[i:i+2])
    ...
    def getlabel(self):
        ...
        for fname in os.listdir(self.test_dir):
            ...
            for line in open(os.path.join(self.test_dir, fname)):
                self.test_label.append(label)     ### ���յõ��� self.train_label��Ԫ�ظ�����ʲô�йأ�
...
if __name__ == '__main__':
    # fPath_train = os.path.abspath('.')+'/F1/train'
    fPath_test = os.path.abspath( '.' ) + '/F1/test'
    lr = LR_Uni_Bi('./F1/train', fPath_test, alpha=0.5)
    lr.buildDic(min1=0, min2=0, sw1=True, sw2=True)
    lr.getlabel()
    lr.setLog('./F1/out')
    lr.SGDwithTFIDF(iter=50)
    import os
    print((os.path.abspath('./F1/train/train_neg.txt')))
